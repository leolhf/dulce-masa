...
function getPeriodoFechas(tipo) {
  ...
  if (tipo === '7')  return { inicio: new Date(hoy.getTime()-7*864e5),  fin: hoy, label:'Últimos 7 días' };
  if (tipo === '30') return { inicio: new Date(hoy.getTime()-30*864e5), fin: hoy, label:'Últimos 30 días' };
  if (tipo === '90') return { inicio: new Date(hoy.getTime()-90*864e5), fin: hoy, label:'Últimos 90 días' };
  if (tipo === 'mes_actual') {
    return { inicio: new Date(hoy.getFullYear(), hoy.getMonth(), 1), fin: hoy, label:'Este mes' };
  }
  if (tipo === 'mes_pasado') {
    return {
...
function renderEstadisticas(){
  ...
  if(periodoActual==='7'||periodoActual==='30'||periodoActual==='mes_actual'){
    ...
    const diasMes     = new Date(hoy2.getFullYear(), hoy2.getMonth()+1, 0).getDate();
...
function _renderStatsLineChart(periodo, ingresosTotal, gastosTotal){
  ...
  if(agrupar === 'dia'){
    ...
    while(cur <= periodo.fin){
      ...
      const s0 = cur.toISOString().split('T')[0];
      const s1 = fin.toISOString().split('T')[0];
