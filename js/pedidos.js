  ...
  lista.innerHTML = filtrados.map(p => {
    ...
    return `<div style="border:1px solid ${borderColor};border-radius:var(--r);padding:12px;margin-bottom:8px;background:${bgColor}">
      ...
      <div style="display:flex;justify-content:space-between;font-size:.78rem;margin-bottom:6px">
        <span>Entrega: <strong>${fe?fe.toLocaleDateString('es-AR'):'—'}</strong> · ${diffTxt}</span>
...
function renderCalendarioPedidos(){
  ...
  const primerDia = createDate(year, month, 1).getDay(); // 0=dom
  const diasMes = createDate(year, month+1, 0).getDate();
  const hoyStr = today();

  // Días de la semana (lunes primero)
  const diasSemana = ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];
  const offset = (primerDia + 6) % 7; // convertir a lunes=0

  let celdas = '';
  // Celdas vacías del mes anterior
  for(let i = 0; i < offset; i++){
    const dPrev = createDate(year, month, -offset+i+1).getDate();
  ...
  for(let d = 1; d <= diasMes; d++){
    ...
    const eventosHTML = eventos.slice(0,3).map(p => {
      ...
      return `<span class="cal-event ${cls}" data-id="${p.id}" onclick="verPedido(+this.dataset.id)" title="${p.cliente}: ${(p.items||[]).map(it=>rec(it.recetaId)?.nombre||'?').join(', ')}">${p.cliente}</span>`;
    }).join('');
