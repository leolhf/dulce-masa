...
function cargarDatosCapital() {
  // Obtener capital calculado del dashboard
  const capitalKPI = document.getElementById('kpi-capital-total');
  const capitalResumen = document.getElementById('resumen-capital');
  
  capitalCalculadoActual = 0;
  if (capitalKPI) {
    const texto = capitalKPI.textContent.replace(/[^0-9.-]/g, '');
    capitalCalculadoActual = parseFloat(texto) || 0;
  } else if (capitalResumen) {
    const texto = capitalResumen.textContent.replace(/[^0-9.-]/g, '');
    capitalCalculadoActual = parseFloat(texto) || 0;
  }
